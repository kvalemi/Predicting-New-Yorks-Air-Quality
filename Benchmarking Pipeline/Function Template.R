...
# Fit a Least Squares to the training data passed to it on all of the explanatory variables
...
# Fit a Stepwise AIC to the training data passed to it on all of the explanatory variables
...
# Fit a Stepwise BIC to the training data passed to it on all of the explanatory variables
...
fit.gam.model = function(data.train, data.valid, Y.valid, response_name) {
  ...
  formula = as.formula(paste(response_name, "~", "s(Solar.R) + s(Wind) + s(Temp) + s(TWcp) + s(TWrat)"))  
...
fit.1layer.nnet = function(data.train.full, data.valid.full, M, K, all.n.hidden, all.shrink, response_name) {
  ...
  print(paste0("-> Nnet - Most Optimal Hyper Parameters: ", optimal.n.hidden, ", ", optimal.shrink, " - MSPE: ", MSPE.nnet))
